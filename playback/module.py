...
# TODO: Find better parameter name than subset, implement it and test it. Maybe use key value pairs?
def playback(*, source_path: str, speed: int, subset: list[str | int] = None,
    ...
    for file in files:
        ...
        # FIXME (Short): Read only relevant columns instead of all columns and order the same as MarineTraffic.
        cols_to_read = ['MMSI', 'IMO', 'STATUS', 'SPEED', 'LON', 'LAT', 'COURSE', 'HEADING', 'DATE', 'TIME']
        ...
        dataframe = dataframe[(dataframe['TIMESTAMP'].dt.time >= start_time) & (dataframe['TIMESTAMP'].dt.time <= stop_time)]
